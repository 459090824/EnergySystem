...
-->                      
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addSystemEquipmentType">添加设备类型</a>
                </div>
            </div>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tr class="first">
                            <td class="text-center">
                             	${systemEquipmentType.systemEquipmentType_Name}
                            </td>
                            <td class="text-center">                           	
                                ${systemEquipmentType.systemEquipmentType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${systemEquipmentType.systemEquipmentType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${systemEquipmentType.systemEquipmentType_ID}" value="${systemEquipmentType.systemEquipmentType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showSystemEquipmentType").bind('click','i', function(e){
                ...
                var systemEquipmentType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(systemEquipmentType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				...
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteSystemEquipmentType/"+systemEquipmentType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
	    		    						   alert("删除成功！");
	    		    						   window.location.href="showSystemEquipmentType"; 
	    			    				    }else{
	    		    		    			      alert("删除失败！");	
	    		    		    			      }
	    		    						return false;
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备类型？")){
                                     window.location.href="editSystemEquipmentType/" + systemEquipmentType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
 ...
 <script type="text/javascript">
        $(function () {
            // toggle form between inline and normal inputs
            $("#addSystemEquipmentType").click(function () {
                $.ajax({
                	...
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddSystemEquipmentType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
