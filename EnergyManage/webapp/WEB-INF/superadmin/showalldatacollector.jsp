...
<head>
	<title>数据采集器列表</title>
	<meta name="keywords" content="显示数据采集器设备" />
	<meta name="description" content="显示所有数据采集器设备" />
...
<body>
        	...
        	<div class="row head">
             ...
             <div class="row filter-block" style="line-height:60px">
                ...
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addDataCollector">添加数据采集器</a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        <thead >
                            <tr>
                                <th class="col-md-1 text-center" >
                                   	<span class="line"></span>采集器名
                                </th>
                                <th class="col-md-1 text-center">
                                    <span class="line"></span>采集器编号
                                </th>
                                <th class="col-md-1 text-center">
                                    <span class="line"></span>采集器类型
                                </th>
                                <th class="col-md-1 text-center">
                                    <span class="line"></span>采集器状态
                                </th>
                                <th class="col-md-1 text-center">
                                    <span class="line"></span>采集器采集数据周期
                                </th>
                                <th class="col-md-1 text-center">
                                    <span class="line"></span>采集器位置
                                </th>
                                <th class="col-md-1 text-center">
                        ...
                        <tbody id="showDataCollector">
                        	<c:forEach items="${dataCollectorUnions}" var="dataCollectorUnion" varStatus="status">                        		
                        <!-- row -->
                        <tr class="first">
                            <td class="text-center">
                             	${dataCollectorUnion.collection_Name}
                            </td>
                            <td class="text-center">                            	
                                ${dataCollectorUnion.collection_Code}                             
                            </td>
                            <td class="text-center">                            	
                                ${dataCollectorUnion.systemEquipmentSubType.systemEquipmentSubType_Name}                             
                            </td> 
                            <td class="text-center">                          	
                                ${dataCollectorUnion.state.state_Name}                             
                            </td> 
                            <td class="text-center">                           	
                                ${dataCollectorUnion.collect_Interval}                             
                            </td> 
                            <td class="text-center">                           	
                                ${dataCollectorUnion.room.room_Name}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${dataCollectorUnion.collection_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEquipment${dataCollectorUnion.collection_ID}" value="${dataCollectorUnion.collection_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showDataCollector").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var collection_ID = e.target.getAttribute('value');
                console.log(th);
	    				...
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteDataCollector/"+collection_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备？")){
                                     window.location.href="editDataCollector/" + collection_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
  ...
  <script type="text/javascript">
        $(function () {
            // toggle form between inline and normal inputs
            $("#addDataCollector").click(function () {
                $.ajax({
                	...
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddDataCollector";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
