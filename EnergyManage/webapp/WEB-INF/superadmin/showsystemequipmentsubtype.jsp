...
-->                      
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addSystemEquipmentSubType">添加设备子类型</a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tbody id="showSystemEquipmentType">
                        	<c:forEach items="${systemEquipmentSubTypeUnions}" var="systemEquipmentSubTypeUnion" varStatus="status">                        		
                        <!-- row -->
                        <tr class="first">
                            <td class="text-center">
                             	${systemEquipmentSubTypeUnion.systemEquipmentSubType_Name}
                            </td>
                            <td class="text-center">                           	
                                ${systemEquipmentSubTypeUnion.systemEquipmentType.systemEquipmentType_Name}                             
                            </td>
                            <td class="text-center">                           	
                                ${systemEquipmentSubTypeUnion.systemEquipmentSubType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${systemEquipmentSubTypeUnion.systemEquipmentSubType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${systemEquipmentSubTypeUnion.systemEquipmentSubType_ID}" value="${systemEquipmentSubTypeUnion.systemEquipmentSubType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showSystemEquipmentType").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var systemEquipmentSubType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(systemEquipmentSubType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				...
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备子类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteSystemEquipmentSubType/"+systemEquipmentSubType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
	    		    						   alert("删除成功！");
	    		    						   window.location.href="showSystemEquipmentSubType"; 
	    			    				    }else{
	    		    		    			      alert("删除失败！");	
	    		    		    			      }
	    		    						return false;
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备类型？")){
                                     window.location.href="editSystemEquipmentSubType/" + systemEquipmentSubType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
 ...
 <script type="text/javascript">
        $(function () {
            // toggle form between inline and normal inputs
            $("#addSystemEquipmentSubType").click(function () {
                $.ajax({
                	url:"judgeonClickAddSystemEquipmentType",
                	type : "GET",
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备子类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddSystemEquipmentSubType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
