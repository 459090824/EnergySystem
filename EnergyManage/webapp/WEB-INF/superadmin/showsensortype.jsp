...
<body>
        ...
        <div id="pad-wrapper" class="adminList">
            ...
            <div class="row filter-block" style="line-height:60px">
                ...
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addSensorType"> 添加传感器类型</a>
                </div>
            </div>
            <!--<div class="row header">
                ...
                <div class="col-md-10 col-sm-12 col-xs-12 pull-right">
                    ...
                    <a href="<%=basePath %>superadmin/onClickAddSensorType" class="btn-flat success pull-right">
                        <span>&#43;</span>
                      		  添加传感器类型
                    </a>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tbody id="showSensorType">
                        	<c:forEach items="${sensorTypes}" var="sensorType" varStatus="status">                        		
                        <!-- row -->
                        <tr class="first">
                            <td class="text-center">
                             	${sensorType.sensorType_Name}
                            </td>
                            <td class="text-center">                           	
                                ${sensorType.sensorType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${sensorType.sensorType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${sensorType.sensorType_ID}" value="${sensorType.sensorType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showSensorType").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var sensorType_ID = e.target.getAttribute('value');
                console.log(th);
	    				...
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteSensorType/"+sensorType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备类型？")){
                                     window.location.href="editSensorType/" + sensorType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
                	...
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddSensorType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
