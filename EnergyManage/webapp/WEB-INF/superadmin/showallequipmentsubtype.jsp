...
<head>
	<title>设备子类型列表</title>
	<meta name="keywords" content="显示所有设备子类型" />
	<meta name="description" content="显示所有设备子类型" />
...
<body>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tbody id="showEquipmentSubType">
                        	<c:forEach items="${equipmentSubTypeUnions}" var="equipmentSubTypeUnion" varStatus="status">                        		
                        <!-- row -->
                        <tr class="first">
                            <td class="text-center">
                             	${equipmentSubTypeUnion.equipmentSubType_Name}
                            </td>
                            <td class="text-center">
                             	${equipmentSubTypeUnion.equipmentType.equipmentType_Name}
                            </td>
                            <td class="text-center">                            	
                                ${equipmentSubTypeUnion.equipmentSubType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${equipmentSubTypeUnion.equipmentSubType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${equipmentSubTypeUnion.equipmentSubType_ID}" value="${equipmentSubTypeUnion.equipmentSubType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showEquipmentSubType").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var equipmentSubType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(equipmentSubType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				url:"<%=basePath %>superadmin/judgedeleteEquipmentSubType",
	    				type:"GET",
	    				//data:{_method:'delete'},
	    				//dataType:"json",
	    				async:false,
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备子类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteEquipmentSubType/"+equipmentSubType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该区域？")){
                                     window.location.href="editEquipmentSubType/" + equipmentSubType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
   ...
   <script type="text/javascript">
        $(function () {
            ...
            $("#addEquipmentSubType").click(function () {
                $.ajax({
                	url:"judgeonClickAddEquipmentSubType",
                	type : "GET",
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备子类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddEquipmentSubType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
