...
<head>
	<title>建筑类型列表</title>
	<meta name="keywords" content="显示所有建筑类型" />
	<meta name="description" content="显示所有建筑类型" />
...
<body>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tr class="first">
                            <td class="text-center">
                             	${buildingType.buildType_Name}
                            </td>
                            <td class="text-center">                            	
                                ${buildingType.buildType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${buildingType.buildType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${buildingType.buildType_ID}" value="${buildingType.buildType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showEnergy").bind('click','i', function(e){
                ...
                var buildType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(buildType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				url:"<%=basePath %>superadmin/judgedeleteBuildingType",
	    				type:"GET",
	    				//data:{_method:'delete'},
	    				//dataType:"json",
	    				async:false,
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该建筑类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteBuildingType/"+buildType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
	    		    						   alert("删除成功！");
	    		    						   window.location.href="allBuildingType"; 
	    			    				    }else{
	    		    		    			      alert("删除失败！");	
	    		    		    			      }
	    		    						return false;
                ...
                if(th == 'edit' ){
                    $.ajax({
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该建筑类型？")){
                                     window.location.href="editBuildingType/" + buildType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
  ...
  <script type="text/javascript">
        $(function () {
            ...
            $("#addBuildingType").click(function () {
                $.ajax({
                	url:"judgeonClickAddBuildingType",
                	type : "GET",
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加建筑类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddBuildingType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
