...
<body>
        	...
        	<div class="row head">
             ...
             <div class="row filter-block" style="line-height:60px">
                ...
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addDataCollectorType"> 添加采集器类型</a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table cellpadding="0" cellspacing="0" border="0" class="" id="example">
                        ...
                        <tbody id="showDataCollectorType">
                        	<c:forEach items="${dataCollectorTypes}" var="dataCollectorType" varStatus="status">                        		
                        <!-- row -->
                        <tr class="first">
                            <td class="text-center">
                             	${dataCollectorType.collectionType_Name}
                            </td>
                            <td class="text-center">                          	
                                ${dataCollectorType.collectionType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${dataCollectorType.collectionType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${dataCollectorType.collectionType_ID}" value="${dataCollectorType.collectionType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showDataCollectorType").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var collectionType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(collectionType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				...
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteDataCollectorType/"+collectionType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备类型？")){
                                     window.location.href="editDataCollectorType/" + collectionType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
 ...
 <script type="text/javascript">
        $(function () {
            // toggle form between inline and normal inputs
            $("#addDataCollectorType").click(function () {
                $.ajax({
                	...
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddDataCollectorType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
