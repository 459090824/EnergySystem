...
<head>
	<title>设备类型列表</title>
	<meta name="keywords" content="显示所有设备类型" />
	<meta name="description" content="显示所有设备类型" />
...
<body>
        ...
        <div id="pad-wrapper" class="adminList">
            ...
            <div class="row filter-block" style="line-height:60px">
                <div class="pull-right" style="padding-bottom:9px;padding-right:15px">
                    <a class="btn-flat success new-product" href="#" id="addEquipmentType">添加设备类型</a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table id="example" cellpadding="0" cellspacing="0" border="0" class="table table-hover">
                        <thead >
                            <tr>
                        ...
                        <tr class="first">
                            ...
                            <td class="text-center">                           	
                                ${equipmentType.equipmentType_Desc}                             
                            </td>                                                  
                            <td class="text-center">
                                <i class="table-edit"  name="edit" value="${equipmentType.equipmentType_ID}" href="#" title="编辑" style="cursor: pointer;"></i> | <i class="table-delete" name="delete" id="deleteEnergyType${equipmentType.equipmentType_ID}" value="${equipmentType.equipmentType_ID}" href="" title="删除" style="cursor: pointer;"></i>
                            </td>
    ...
    <script type="text/javascript">
        $(function(){
            $("#showEquipmentType").bind('click','i', function(e){
                var th = e.target.getAttribute('name');
                var equipmentType_ID = e.target.getAttribute('value');
                console.log(th);
                console.log(equipmentType_ID);
                if (th == 'delete') {
                	$.ajax({
	    				url:"<%=basePath %>superadmin/judgedeleteEquipmentType",
	    				type:"GET",
	    				//data:{_method:'delete'},
	    				//dataType:"json",
	    				async:false,
	    				success:function(msg){
	    					if(msg.success == '1'){	 
	    						if(confirm("你确信要删除该设备类型？")){	    		    							
	    							$.ajax({
	    			    				url:"deleteEquipmentType/"+equipmentType_ID,
	    			    				type:"DELETE",
	    			    				async:false,
	    			    				success:function(msg){		    					
	    		    					    if(msg.success == '1'){
	    		    						   alert("删除成功！");
	    		    						   window.location.href="allEquipmentType"; 
	    			    				    }else{
	    		    		    			      alert("删除失败！");	
	    		    		    			      }
	    		    						return false;
                        ...
                        success:function(msg){
                            if(msg.success == '1'){
                                 if(confirm("你确信要编辑该设备类型？")){
                                     window.location.href="editEquipmentType/" + equipmentType_ID;                         
                                 }else{
                                     alert("已取消！");
                                     return false;
                                 }
  ...
  <script type="text/javascript">
        $(function () {
            ...
            $("#addEquipmentType").click(function () {
                $.ajax({
                	url:"judgeonClickAddEquipmentType",
                	type : "GET",
                	success:function(msg){  
                		if(msg.success == '1'){
                			if(confirm("你确信要添加设备类型？")){
   							 window.location.href="<%=basePath %>superadmin/onClickAddEquipmentType";    						
   						 }else{
   							 alert("已取消！");
   							 return false;
   						 }
